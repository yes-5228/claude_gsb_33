...
| 问题上报 | `/issues`、`/issues/:id` | 问题上报（可关联巡查记录）、分类/程度/期限、整改流程流转、整改轨迹时间线、超期预警、追加跟进记录、按筛选条件导出 CSV 清单 |
...
| GET | `/issues/export` | 按列表同样的筛选条件导出问题清单（CSV，含「是否超期」列） |
...
- **超期预警**：超期判定全系统只有一套口径（`issue_service.overdue_conditions` / `is_overdue`）——状态处于未闭环（待整改/整改中/待验收）且整改期限早于当前时刻即为超期；无期限或已闭环（已完成/已关闭）的问题不算超期。列表筛选、详情与列表行的「已超期」标识（接口统一下发 `is_overdue` 字段）、看板超期统计、CSV 导出清单均使用该口径，结论一致；`overdue=true/false` 互为补集。超期数量按查询时刻实时计算，历史月份已统计的超期数不回溯重算。
